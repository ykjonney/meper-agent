...
/**
 * 将 xyflow Edge 数组转回 WorkflowEdge 数组。
 */
export function fromXyflowEdges(
  xyflowEdges: Edge[],
): WorkflowEdge[] {
  return xyflowEdges.map((xe) => ({
    ...(xe.data?.workflowEdge as WorkflowEdge),
    source: xe.source,
    target: xe.target,
    label: (xe.label as string) ?? '',
    condition: (xe.data?.workflowEdge as WorkflowEdge)?.condition ?? null,
  }))
}
 ...
 * @param source 连线的 source node_id
 * @param target 连线的 target node_id
 * @param label 可选标签
 * @param condition 可选条件表达式
 * @returns 修改后的新 nodes 数组
 */
