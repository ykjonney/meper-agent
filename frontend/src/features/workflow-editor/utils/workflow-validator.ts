 ...
 * @param nodes 工作流节点列表
 * @param edges 工作流边列表
 * @param hasUnsavedChanges 是否有未保存的修改
 * @returns 验证结果（包含 errors 和 warnings）
 */
 ...
 *
 * 检查：
 * - 引用的 node_id 是否存在于工作流中
 * - 引用的 field 是否存在于该节点的输出字段
 */
