...
import { Button, Popover, Tag, Input, Badge, Tooltip } from 'antd'
...
export default function VariableSelector({
  ...
  const renderFieldTag = (field: UpstreamField, group: UpstreamGroup) => {
    ...
    return (
      <Tooltip key={field.name} title={tooltipContent} placement="topLeft">
        <Tag
          className="!m-0 !cursor-pointer !text-[11px] !px-1.5 !py-0.5 !rounded hover:!opacity-80 transition-opacity !border-0 !inline-flex !items-center !gap-1"
          color="blue"
    ...
    <div className="w-72 max-h-72 overflow-y-auto">
      {upstreamGroups.length === 0 ? (
        <div className="text-xs text-[#94A3B8] text-center py-4">
          <CodeOutlined className="text-base mb-2 block" />
          暂无可用变量
          ...
          {upstreamGroups.map((group) => (
            <div key={group.nodeId} className="mb-2.5 last:mb-0">
              ...
              <div className="flex items-center gap-1.5 mb-1.5 px-0.5">
                ...
                <span className="text-[10px] text-[#94A3B8] font-mono ml-auto shrink-0">
                  {group.nodeId}
                </span>
              </div>
              {/* 字段 Tag 列表 */}
              <div className="flex flex-wrap gap-1.5">
                {group.fields.map((field) => renderFieldTag(field, group))}
              </div>
            </div>
          ))}
        </div>
        ...
        <Popover
          ...
          title={
            <div className="flex items-center justify-between text-xs">
              <span className="font-medium text-[#0F172A]">选择变量</span>
