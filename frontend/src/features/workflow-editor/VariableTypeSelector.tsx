  ...
  ([key, cfg]) => ({
    ...
    label: (
      <div className="flex items-center gap-2">
        ...
        <span>{cfg.label}</span>
        <span className="text-[10px] text-[#94A3B8]">({cfg.description})</span>
      </div>
    ),
  }),
)
...
export default function VariableTypeSelector({
  ...
  const handleTypeChange = (newType: VariableTypeName) => {
    const defaults = getDefaultConstraints(newType)
    onChange({
      ...value,
      type: newType,
    ...
    <div className="space-y-3">
      ...
      {showMeta && (
        <>
          ...
          <div>
            <label className="block text-xs text-[#64748B] mb-1">显示标签</label>
            <Input
              size="small"
              value={value?.label ?? ''}
      ...
      {constraintFields.length > 0 && (
        <div className="border border-gray-100 rounded-lg p-2.5 bg-gray-50">
          <p className="text-[10px] text-[#64748B] font-medium mb-2">约束</p>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-[#94A3B8]">{typeConfig?.description}</span>
      </div>
    </div>
  )
}
