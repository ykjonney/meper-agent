/**
 * Global hook that bridges WebSocket events to TanStack Query and the
 * Notification Store. Call once in the App root (after auth is established).
 *
 * - `task_status` → invalidate the new-status list, the old-status list, and
 *   the task detail query (replaces refetchInterval polling on the board).
 * - `notification` → push into the notification store (dedup by id).
 *
 * Mirrors frontend/src/hooks/use-task-realtime.ts.
 */
