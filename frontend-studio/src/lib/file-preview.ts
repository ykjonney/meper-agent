/**
 * 文件预览能力判断 — 决定 TaskOutputFiles 如何渲染预览。
 *
 * 仅支持图片和文本类内嵌预览；其他类型走"请下载"提示。
 * 保持最小依赖：不做扩展名 fallback / 字节嗅探。
 *
 * 移植自 frontend/src/lib/file-preview.ts。
 */
 ...
 */
export function getPreviewKind(mime: string | undefined | null): PreviewKind {
  if (!mime) return 'none'
