...
import { Button, Table, Modal, Tag, Tooltip, type TableColumn } from '../../components/ui'
...
export default function VariableListEditor({
  ...
  const columns: TableColumn<VariableRow>[] = [
    {
      ...
      key: 'name',
      render: (name: unknown) => (
        <code className="text-xs font-mono text-[#fafafa]">{String(name)}</code>
      ),
    },
    {
      title: '标签',
    ...
    },
    {
      ...
      render: (type: unknown) => (
        ...
        >
          {getTypeIcon(String(type))} {getTypeLabel(String(type))}
        </Tag>
      ),
    },
    {
      ...
      key: 'actions',
      width: 80,
      render: (_value: unknown, _record: VariableRow, index: number) => (
  ...
  const dataSource: VariableRow[] = value.map((v) => ({
    name: v.name,
    label: v.label,
    type: v.type,
  }))
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <label className="text-xs text-slate-400 font-medium">输出变量</label>
        {!readonly && (
          <Button
            type="dashed"
            icon={<Plus size={12} />}
      ...
      {value.length > 0 ? (
        ...
        <div className="text-[10px] text-[#71717a] text-center py-3 border border-dashed border-[#27272a] rounded">
