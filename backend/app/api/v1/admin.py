...
async def list_users(
    ...
    # Strip password_hash from each document
    users = [
        UserResponse(
            id=doc["_id"],
...
async def create_user(
    ...
    doc = await UserService.create_user_by_admin(
        ...
        role=body.role.value if isinstance(body.role, UserRole) else body.role,
    )
    return UserResponse(
        id=doc["_id"],
        username=doc["username"],
...
async def update_user(
    ...
    if body.role is not None:
        updates["role"] = body.role.value if isinstance(body.role, UserRole) else body.role
