...
class TestBuildThinkingKwargs:
    """Test _build_thinking_kwargs returns correct params per provider."""
    ...
    def test_openai_reasoning_model(self):
        result = _build_thinking_kwargs("o3-mini", "openai", True, None)
        assert result == {"reasoning_effort": "high"}

    def test_openai_o1_reasoning(self):
        result = _build_thinking_kwargs("o1-preview", "openai", True, None)
        assert result == {"reasoning_effort": "high"}

    def test_openai_o4_reasoning(self):
        result = _build_thinking_kwargs("o4-mini", "openai", True, None)
        assert result == {"reasoning_effort": "high"}

    def test_openai_non_reasoning_model_ignored(self):
        """GPT-4o does not support reasoning_effort."""
        result = _build_thinking_kwargs("gpt-4o", "openai", True, None)
        assert result == {}

    def test_openai_non_reasoning_model_with_prefix(self):
        """gpt-o1 should not match (it starts with gpt-, not o1-)."""
        result = _build_thinking_kwargs("gpt-o1", "openai", True, None)
        assert result == {}

    def test_unknown_provider_ignored(self):
        result = _build_thinking_kwargs("some-model", "custom", True, None)
        assert result == {}
