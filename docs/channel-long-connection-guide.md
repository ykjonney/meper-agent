...
IM 侧一个聊天窗口 ↔ agent 侧一条持续会话(对齐 Coze / OpenClaw 等产品惯例):

- **会话域**:身份编码为 `channel:{channel_id}:{platform_chat_id}`。单聊 =
  每个窗口一条连续会话;群聊 = 全群共享一条对话流(群内所有人上下文互通)。
- **会话延续**:处理消息时复用该身份域内最近活跃的 session,与 Web 端
  多轮对话走完全相同的链路(checkpointer 历史、per-session token 预算、
  llm_summary 压缩自动生效)。
- **空闲重置**:`CHANNEL_SESSION_IDLE_RESET_MINUTES`(默认 24 小时)内没有
  新消息则自动开新会话;设为 `0` 表示永不因空闲重置。
- **最大寿命**:`CHANNEL_SESSION_MAX_AGE_HOURS`(默认 24)——活满后下一条消息
  自动开新会话(按天滚动),防止持续活跃的聊天无限累积;`0` = 不限寿命。
- **上下文压缩**:多轮记忆由 checkpointer + 五级压缩管线承载(工具压缩→
  后台 LLM 摘要→硬上限保护),模型看到的上下文永远有界,会话可以无限延续。
- **messages 明细**:渠道会话默认不落 `messages` 集合(没有消费方,纯占存储);
  `CHANNEL_PERSIST_MESSAGES=true` 可为审计打开。
- **手动重置**:发送 `#新话题` / `/new` / `/reset`(精确匹配)立即开新会话,
  机器人回复确认,指令本身不会发给 agent。
- **群聊用户感知**:群消息送入 agent 前会加 `[发送者昵称] ` 前缀——全群仍共享
  一条会话流(可跨用户接话),但 agent 能区分谁在说话。飞书事件不含昵称,
  前缀自动跳过。重置指令匹配仍用原始文本,不受前缀影响。
- **旧会话保留**:轮换后的渠道旧会话(session+记忆+工作区)保留
  `CHANNEL_SESSION_RETENTION_DAYS` 天(默认 7)供排障,超期由每日 04:00
  定时任务级联清理;Web 会话不受影响。
- **群聊触发规则**:
  - 钉钉:平台侧只推送被 @ 的群消息,天然不刷屏。
  - 飞书:`CHANNEL_LARK_GROUP_MENTION_ONLY=true`(默认)时仅处理以 @ 机器人
    占位符开头的消息,其余群消息忽略;@ 占位符会从正文清洗掉。
    注意:平台无法在事件里区分 @ 的是机器人还是人,采用"首个 @ 即视为
    @ 机器人"的启发式——群里先 @ 某人再说话的消息也会被响应,若困扰可
    关闭该开关退回"处理全部群消息"。
