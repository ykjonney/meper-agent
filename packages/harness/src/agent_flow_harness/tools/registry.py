...
class ToolRegistry:
    ...
    def register(self, tool: BaseTool | CommunityTool) -> None:
        ...
        if self._is_community_tool(tool):
            self._community_tools[tool.name] = tool  # type: ignore[union-attr]
            logger.info("tool_registry_community_registered", name=tool.name)  # type: ignore[union-attr]
        else:
    ...
    @staticmethod
    def _is_community_tool(tool: Any) -> bool:
